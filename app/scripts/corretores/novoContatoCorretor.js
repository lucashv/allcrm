(function() {
  ...
  function NovoContatoCorretor($scope,
    ...
    $scope.salvaContato = function(contato, emails, telefones) {

      var novoContato = montaContato(contato, emails, telefones);
      $scope.loadingForm = ContatosService.create(novoContato).then(function(response) {
        ...
        ContatosService
          ...
          .then(function(success) {
            if (success.total_items > 0) {
              ...
              visualizarContato(success._embedded.contatos[0]);
              $scope.aviso = {
                tipo: 'Telefone',
